...
项目使用宿主机部署：系统安装 `ffmpeg`，Python 依赖安装在虚拟环境中，并由 `systemd` 守护 API 和调度器进程。
...
- 短于 `MIN_VALID_RECORDING_DURATION_SECONDS` 的片段会标记为失败，不会当作正常成品发送
- 只要有主播处于录制中，巡检频率会自动切到 `ACTIVE_RECORDING_CHECK_INTERVAL`，比常规巡检更快发现断流后的重开

如果你希望失败的短片段文件也直接删掉，可以把 `AUTO_DELETE_FAILED_RECORDINGS=True`。否则它们会和正常录制一样，按保留天数统一清理。
